...
<div><div>Related Topics</div><div><li>数组</li><li>双指针</li><li>二分查找</li></div></div><br><div><li>👍 631</li><li>👎 0</li></div>
*/
...
class Solution {
    public int[] twoSum(int[] nums, int target){
        ...
        while (headIndex != tailIndex) {
            int temp = nums[headIndex] + nums[tailIndex];
            if (temp == target) return new int[]{headIndex+1, tailIndex+1};
            if (temp < target) headIndex++;
            if (temp > target) tailIndex--;
        }
        return null;
    }
}
